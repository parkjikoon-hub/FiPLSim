"""
베르누이 MC 배치 시뮬레이션 — 30개 조건
5 p_b x 3 유량 x 2 비드 높이 = 30개 조건, 각 10,000회 MC
...
OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "MC_Bernoulli_Batch")
os.makedirs(OUT_DIR, exist_ok=True)

print("=" * 70)
print("  FiPLSim Bernoulli MC Batch Simulation")
print(f"  {len(P_VALUES)} p x {len(FLOW_RATES)} flows x {len(BEAD_HEIGHTS)} beads = "
      f"{len(P_VALUES)*len(FLOW_RATES)*len(BEAD_HEIGHTS)} conditions x {N_ITERATIONS:,} iterations")
print("=" * 70)

# ── 요약 테이블 수집 ──
summary_rows = []
condition_idx = 0
total_conditions = len(FLOW_RATES) * len(BEAD_HEIGHTS) * len(P_VALUES)
...
summary_path = os.path.join(OUT_DIR, "Bernoulli_Summary.csv")
df_summary.to_csv(summary_path, index=False, encoding="utf-8-sig")

# ── Excel (요약 + 조건별 피벗) ──
excel_path = os.path.join(OUT_DIR, "Bernoulli_Batch_Results.xlsx")
with pd.ExcelWriter(excel_path, engine="openpyxl") as w:
    # Sheet 1: 전체 요약 (30행)
    df_summary.to_excel(w, sheet_name="전체 요약", index=False)

    # Sheet 2: 평균 수압 피벗 (유량 x 비드 x p_b)
    pivot_mean = df_summary.pivot_table(
        index=["유량 (LPM)", "비드 높이 (mm)"],
        columns="p_b",
        values="평균 수압 (MPa)",
    )
